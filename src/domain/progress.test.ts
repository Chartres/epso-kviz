...
function q(id: number, cat: Question['cat'] = 'AI Engineering & Operations'): Question {
  return {
    id,
    cat,
    q: `q${id}`,
...
describe('summary', () => {
  it('aggregates per category against the full catalogue', () => {
    const all = [q(1), q(2), q(3, 'AI Act & Governance')]
    let p = emptyProgress()
    for (let i = 0; i < MASTERY_STREAK; i++) p = recordAnswer(p, 1, true, i)
    ...
    expect(sum.byCategory['AI Engineering & Operations']).toMatchObject({
      total: 2,
      mastered: 1,
    })
    expect(sum.byCategory['AI Act & Governance']).toMatchObject({
      total: 1,
      seen: 1,
      mastered: 0,
    })
